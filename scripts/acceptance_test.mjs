import { DocumentProcessingService } from '../src/lib/document-processing/service.js';
import { DbRepository } from '../src/lib/repository/db.js';
import { detectDuplicate } from '../src/lib/duplicate-detector.js';
import { evaluatePassportHealth } from '../src/lib/passport-health.js';
...
async function runAcceptanceTests() {
  ...
  const samplePassportText = `
    ...
    P<IDNAHMAD<<MUHAMMAD<<<<<<<<<<<<<<<<<<<<<<<<
    C1234567<8IDN8810158M3401104<<<<<<<<<<<<<<04
  `;
  ...
  assert(extractionPassport.fields.passport_number === 'C1234567', 'Extracts Passport Number C1234567');
  assert(extractionPassport.fields.passport_name?.includes('MUHAMMAD'), 'Extracts Passport Full Name');
  assert(extractionPassport.fields.birth_date === '1988-10-15', 'Extracts Date of Birth 1988-10-15');
  ...
  const selectedMember = extractionKk.fields.members.find(m => m.name.includes('FATIMAH')) || extractionKk.fields.members[1];
